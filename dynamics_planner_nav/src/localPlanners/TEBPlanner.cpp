// TEBPlanner.cpp — Wrapper for official TEB local planner from move_base
// This version uses teb_local_planner library instead of custom implementation
...
namespace Antipatrea {
    ...
    TEBPlanner::TEBPlanner() {
        ...
        teb_config_.robot.max_vel_theta = 1.5;
        ...
        teb_config_.obstacles.inflation_dist = 1;
        ...
        ROS_INFO("TEBPlanner initialized with official teb_local_planner library");
    }
    ...
    bool TEBPlanner::Solve(int nrIters, double dt, bool &canBeSolved) {
    ...
    void TEBPlanner::commonParameters(Robot_config &robot) {
        local_goal = robot.getLocalGoalCfg();
        local_paths = robot.local_paths;
    ...
    void TEBPlanner::normalParameters(Robot_config &robot) {
        teb_config_.robot.max_vel_x = robot.max_vel_x;
        teb_config_.optim.weight_obstacle = 30.0;
    }

    void TEBPlanner::lowSpeedParameters(Robot_config &robot) {
        teb_config_.robot.max_vel_x = std::min(robot.max_vel_x, 0.75);
        teb_config_.optim.weight_obstacle = 50.0;  // More conservative
    }

    bool TEBPlanner::handleNoMapPlanning(geometry_msgs::Twist &cmd_vel) {
    ...
    bool TEBPlanner::handleNormalSpeedPlanning(geometry_msgs::Twist &cmd_vel, double dt) {
        normalParameters(*robot);
        ...
        if (!success) {
            ROS_WARN("TEB planning failed, stopping");
    ...
    bool TEBPlanner::handleLowSpeedPlanning(geometry_msgs::Twist &cmd_vel, double dt) {
    ...
    bool TEBPlanner::handleAbnormalPlaning(geometry_msgs::Twist &cmd_vel, double dt) {
    ...
    bool TEBPlanner::callTEBPlanner(geometry_msgs::Twist &cmd_vel) {
    ...
    void TEBPlanner::buildGlobalPlan(std::vector<geometry_msgs::PoseStamped> &plan) {
        plan.clear();
        ...
        // Use local_paths from global planner (in baseline frame, transform to odom)
        if (!local_paths.empty()) {
            // Add ALL points from local_paths to give TEB the full global plan
            ...
            for (size_t i = 0; i < local_paths.size(); ++i) {
                ...
                double x_baseline = local_paths[i][0];
                double y_baseline = local_paths[i][1];
                ...
                if (i + 1 < local_paths.size()) {
                    double dx = local_paths[i + 1][0] - local_paths[i][0];
                    double dy = local_paths[i + 1][1] - local_paths[i][1];
        ...
        // Always add local_goal as final goal (even if local_paths is empty)
    ...
    void TEBPlanner::visualizeTEBTrajectory() {
    ...
    void TEBPlanner::publishCommand(geometry_msgs::Twist &cmd_vel, double linear, double angular) {
