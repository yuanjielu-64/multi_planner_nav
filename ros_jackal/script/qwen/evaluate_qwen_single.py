...
# script/qwen/evaluate_qwen_single.py -> script/ -> ros_jackal/
...
from script.qwen.qwen_client import QwenClient
...
def main(world_id, planner='DWA', total_worlds=300, runs_per_world=2, qwen_url="http://localhost:5000", ros_port=11311, save_image=True, algorithm_name='Qwen', num_trials=1):
    ...
    while ep < num_trials:
        ...
        while not done:
            ...
            image_name = f"VLM_{current_img_id:06d}.png"
            image_path = os.path.join(file_sync.actor_dir, image_name)

            if os.path.exists(image_path):
                result = vlm_client.infer_from_server(image_path, linear_vel, angular_vel)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--qwen_url', type=str, default="http://192.168.1.175:6000", help='Qwen service URL')
    ...
    parser.add_argument('--algorithm_name', type=str, default='Qwen',
                        help='Algorithm name for identification (default: Qwen)')
    ...
    # 从 policy_name 提取算法类型 (e.g., "ddp_qwen" -> "DDP")
    ...
    main(world_id, planner=planner, total_worlds=args.total_worlds, qwen_url=args.qwen_url, ros_port=args.ros_port, save_image=args.save_image, algorithm_name=args.algorithm_name, num_trials=args.num_trials)