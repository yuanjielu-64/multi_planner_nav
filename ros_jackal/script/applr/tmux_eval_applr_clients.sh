...
echo "=================================================="
echo "  启动 APPLR 评估客户端 - Planner: ${PLANNER^^}"
echo "=================================================="
...
if [[ "$USE_TMUX" == "false" ]]; then
    ...
    cd "${ROS_JACKAL_DIR}"
    exec bash script/applr/eval_batch_worlds_singularity.sh \
        --id ${CONTAINER_ID} \
        --policy ${POLICY_NAME} \
        --start ${START_WORLD} \
        --end ${END_WORLD} \
...
for session in "${SESSIONS_TO_START[@]}"; do
    ...
    tmux new-session -d -s "${TMUX_NAME}" \
        "cd ${ROS_JACKAL_DIR} && bash script/applr/eval_batch_worlds_singularity.sh \
            --id ${CONTAINER_ID} \
            --policy ${POLICY_NAME} \
            --start ${START_WORLD} \
            --end ${END_WORLD} \
...
echo "  或使用脚本: bash script/applr/kill_tmux_sessions.sh"
echo ""
